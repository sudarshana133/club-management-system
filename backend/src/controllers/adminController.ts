...
const addMembers = async (req: CustomReq, res: Response) => {
  ...
  if (uIds.length === 0)
    return res.status(403).json({ msg: "User not present to add as members" });
  if (req.role === "STUDENT")
...
const removeMember = async (req: CustomReq, res: Response) => {
  ...
  if (req.role === "STUDENT")
    return res.status(403).json({ msg: "You are not admin" });
  if (uIds.length === 0)
    return res.status(403).json({ msg: "There are not members to remove" });
  try {
    ...
    res
      .status(200)
      .json({ msg: "deleted members succesfully", MembersNotPresent });
  } catch (error: any) {
    res.status(500).json({ msg: "error" + error.message });
  }
};
const searchMembers = async (req: Request, res: Response) => {
  ...
  if (!emailName)
    return res.status(200).json({ msg: "No emailName specified." });
  try {
